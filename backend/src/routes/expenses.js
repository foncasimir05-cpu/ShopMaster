...
router.get('/', async (req, res, next) => {
  try {
    ...
    const countRow = await dbGet(db, `SELECT COUNT(*) as total FROM expenses ${where}`, params);
...
router.get('/summary', async (req, res, next) => {
  try {
    ...
    if (startDate) { where += ' AND date >= ?'; params.push(startDate); }
    if (endDate)   { where += ' AND date <= ?'; params.push(endDate); }
    const total = await dbGet(db, `SELECT COALESCE(SUM(amount),0) as total FROM expenses ${where}`, params);
    const byCategory = await dbAll(db,
...
router.post('/', [...v.createExpense, validate], async (req, res, next) => {
  try {
    ...
    res.status(201).json(await dbGet(db, 'SELECT * FROM expenses WHERE id = ?', [id]));
  } catch (err) { next(err); }
});

// PUT /api/v1/expenses/:id
router.put('/:id', [...v.updateExpense, validate], async (req, res, next) => {
  try {
    const { amount, category, description, date } = req.body;
    const db = getDb();
    const exp = await dbGet(db, 'SELECT id FROM expenses WHERE id = ? AND tenant_id = ?', [req.params.id, req.shopId]);
    if (!exp) return res.status(404).json({ error: 'Expense not found' });
    await dbRun(db,
      'UPDATE expenses SET amount=?, category=?, description=?, date=? WHERE id=?',
      [Number(amount), category || 'Other', description ?? null, date, req.params.id]
    );
    res.json(await dbGet(db, 'SELECT * FROM expenses WHERE id = ?', [req.params.id]));
  } catch (err) { next(err); }
});
