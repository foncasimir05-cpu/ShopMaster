  ...
  try {
    ...
    db.prepare(
      `INSERT INTO products (id, tenant_id, name, sku, barcode, price, cost, stock, category)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(id, req.user.tenantId, name, sku, barcode, price ?? 0, cost ?? 0, stock ?? 0, category);
...
router.put('/:id', (req, res, next) => {
  try {
    ...
    ).run(name, sku, barcode, price, cost, stock, category, req.params.id, req.user.tenantId);
