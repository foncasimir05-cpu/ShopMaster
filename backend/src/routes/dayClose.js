...
router.get('/summary', async (req, res, next) => {
  try {
    ...
    const cogsRow = await dbGet(db, `
      SELECT COALESCE(SUM(si.cost_price * si.quantity),0) as total_cogs
      FROM sale_items si JOIN sales s ON si.sale_id = s.id
      WHERE s.tenant_id=? AND TO_CHAR(s.created_at, 'YYYY-MM-DD')=? AND s.status='completed'
    `, [req.shopId, date]);
...
router.post('/', requireRole('admin', 'owner'), async (req, res, next) => {
  try {
    ...
    res.status(201).json(await dbGet(db, 'SELECT * FROM day_closures WHERE id = ?', [id]));
  } catch (err) { next(err); }
});
