...
router.post('/register-shop', async (req, res, next) => {
  try {
    ...
    db.transaction(() => {
      db.prepare('INSERT INTO tenants (id, name) VALUES (?, ?)').run(shopId, shopName);
      db.prepare(
        'INSERT INTO users (id, tenant_id, name, email, password, role) VALUES (?, ?, ?, ?, ?, ?)'
      ).run(userId, shopId, ownerName, email, hash, 'admin');
    })();
...
router.post('/refresh', (req, res, next) => {
  try {
    ...
    if (!stored || new Date(stored.expires_at) < new Date()) {
      if (stored) db.prepare('DELETE FROM refresh_tokens WHERE token = ?').run(refreshToken);
